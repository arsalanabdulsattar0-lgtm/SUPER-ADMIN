...
import { Input, TextArea, ComboBox, ScrollArea } from '../../components/ui/FormControls';
...
const ReturnInvoiceEditor: React.FC<Props> = ({ data, onChange, onSave, onViewChange, onPrint }) => {
  ...
  // Pre-fill selectedCustomerId if data has customerName
  useEffect(() => {
    if (data.customerName) {
      const match = sampleCustomers.find(c => c.name === data.customerName);
      if (match) {
  ...
  const handleDownloadExcel = () => {
    ...
    link.setAttribute("download", `Return_Invoice_${data.invoiceNumber || 'draft'}.csv`);
  ...
  const handleSave = () => {
    if (onSave) {
      // Force 'Return' type on save
      onSave({ ...data, type: 'Return' });
    } else {
  ...
  const handlePrint = () => {
    ...
    if (element) {
      const runHtml2Pdf = () => {
        const opt = {
          margin: 0,
          filename: `Return_Invoice_${data.invoiceNumber || 'Draft'}.pdf`,
            ...
            <Card className="flex-1 relative z-40 p-0" style={{ borderColor: '#E2E8F0', boxShadow: 'none' }}>
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                  ...
                  <div className="lg:col-span-2">
                    <Input variant="compact" label="Reference Invoice" placeholder="e.g. INV-000248" value={data.reference}
                ...
                <table className={`w-full relative table-fixed ${tableMinW}`}>
                  <thead className="sticky top-0 bg-white z-20">
                    <tr className="text-[10px] font-black tracking-widest bg-white">
                      ...
                      <th className={`px-3 py-2.5 text-left ${unitWidth || 'w-16'} border-b border-[#E2E8F0] whitespace-nowrap`}>Unit</th>
                      <th className={`px-3 py-2.5 text-left ${detailsWidth} border-b border-[#E2E8F0] whitespace-nowrap`}>Details</th>
                      <th className={`px-3 py-2.5 text-left ${qtyWidth} border-b border-[#E2E8F0] whitespace-nowrap`}>Returned Qty</th>
      ...
      <div id="local-printable-container" className="printable-invoice-container">
        <div className="printable-header-flex">
          ...
          <div className="printable-header-right">
            <h2 className="printable-invoice-label">RETURN INVOICE</h2>
