...
import {
  ...
  CheckCircle, AlertCircle, Upload, X, Image, ArrowUpDown, Printer, Box,
} from 'lucide-react';
...
export const CompanyModule: React.FC<CompanyModuleProps> = ({ brand }) => {
  ...
  const [defaultBranchName, setDefaultBranchName] = useState('Head Office');
  ...
  const openAdd = () => {
    ...
    setDefaultBranchName('Head Office');
    setShowForm(true);
  };
  ...
  const openEdit = (c: Company) => {
    ...
    setForm({
      ...
      logo: c.logo || '',
      max_companies_allowed: c.max_companies_allowed ?? 1,
      max_branches_allowed: c.max_branches_allowed ?? 5,
    });
    setShowForm(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      setCompanies(prev => [...prev, { id: newCompanyId, ...form }]);
      if (defaultBranchName.trim()) {
        const generatedCode = defaultBranchName.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5) || 'BR';
        setBranches(prev => [...prev, {
          id: `br${Date.now()}`,
          companyId: newCompanyId,
          name: defaultBranchName.trim(),
          code: generatedCode,
          address: form.city || '',
          is_head_office: true
        }]);
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-[calc(100vh-112px)] pb-2">
      {/* Header */}
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-black" style={{ color: brand.dark }}>Company List</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">{companies.length} companies found · Last updated just now</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="white" icon={Printer}>Print</Button>
          <Button
            variant="white"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className="bg-white rounded-xl p-4 border shadow-sm transition-all group cursor-default"
            style={{ borderColor: '#E2E8F0' }}
          >
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[12px] font-bold text-slate-800 tracking-wide">{stat.label}</p>
      ...
      <SectionCard
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-semibold bg-white/20 px-2 py-0.5 rounded-full">{paginated.length} companies</span>
          </div>
        }
          ...
          <div className="flex items-center gap-3">
            <div className="w-64 relative">
              ...
              <input
                ...
                className="w-full bg-white/10 border border-white/20 text-white placeholder-white/60 text-sm rounded-lg pl-9 pr-4 py-1.5 focus:outline-none focus:ring-2 focus:ring-white/30 transition-all"
              />
            </div>
            <button className="flex items-center gap-2 text-white bg-white/10 hover:bg-white/20 border border-white/20 px-3 py-1.5 rounded-lg text-sm font-medium transition-all">
      ...
      >
        <div className="overflow-auto w-full relative custom-scrollbar min-h-0">
        ...
        {totalPages > 0 && (
          <div
            className="px-4 py-1.5 border-t flex items-center justify-between shrink-0"
      ...
      >
        <div className="space-y-5">
          ...
          {/* Section 4.5: Quotas & Limits */}
          <div className="space-y-1.5">
            <SectionHeader title="Quotas & Limits" icon={Box} />
            <Card className="p-4" style={{ borderColor: '#E2E8F0', boxShadow: 'none' }}>
              <div className="grid grid-cols-2 gap-4">
                <Input
                  label="Max Companies Allowed"
                  type="number"
                  variant="compact"
                  placeholder="e.g. 1"
                  value={form.max_companies_allowed}
                  onChange={e => setForm({ ...form, max_companies_allowed: Number(e.target.value) })}
                />
                <Input
                  ...
                  onChange={e => setForm({ ...form, max_branches_allowed: Number(e.target.value) })}
                />
              </div>
            </Card>
          </div>
